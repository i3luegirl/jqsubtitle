...
Create **SRT subtitles from video or audio in one click** on Windows.
...
- **Song sections get subtitles too**: paste the lyrics and JQSubtitle fills in the musical or theme-song part that speech recognition drops entirely. It finds the section, separates the vocals for just that part, transcribes it again and lays the real lyrics on top. Timings come from measurement, never from guesswork; the subtitles from before the change are kept as `_nosong.srt`
...
- **SRT output** with a UTF-8 marker, so the text shows correctly in every player including GOM, PotPlayer and KMPlayer
...
- **Local AI (free, offline)** — select it in the app and click Install; it sets up
  Ollama and a ~7.6 GB model (Gemma 4 12B). An NVIDIA GPU with 12 GB+ VRAM is recommended.
...
Bug reports, ideas, and "this annoyed me" notes are all welcome:
[GitHub Issues](https://github.com/i3luegirl/jqsubtitle/issues)

## Support

JQSubtitle is free and always will be. If it saved you time and you feel like it:
[PayPal](https://paypal.me/jqpark) ☕

---

# 한국어 소개

영상/음성 파일에서 **SRT 자막을 한 번에 자동 생성**하는 Windows용 프로그램입니다.
...
- **노래 구간도 자막이 나옵니다**: 가사를 넣으면, 음성 인식이 통째로 놓치는 뮤지컬·주제가 구간을 찾아서 채웁니다. 구간을 알아서 찾고 그 부분만 보컬을 분리해 다시 받아쓴 뒤 진짜 가사를 얹습니다. 시각은 짐작이 아니라 실측이고, 바꾸기 전 자막은 `_nosong.srt` 로 남습니다
- **응답 검증**: 단어 번호가 빠지거나 겹치거나 범위를 벗어난 응답은 거부하고 원본 유지 (사유는 로그에 기록)
- **SRT 출력** — 파일 앞에 UTF-8 표시를 붙여서 곰·팟·KMPlayer 등 어느 플레이어에서도 글자가 안 깨집니다
- **AI 엔진 3종** — 프로그램에서 선택:
...
- **로컬 AI (무료·오프라인)** — 프로그램에서 선택하고 설치 버튼을 누르면 Ollama와
  모델(~7.6GB, Gemma 4 12B)이 설치됩니다. NVIDIA GPU VRAM 12GB 이상 권장. 품질은 무료 API에
...
버그 제보, 아이디어, "이게 불편하더라" 같은 의견 다 환영합니다:
[GitHub Issues](https://github.com/i3luegirl/jqsubtitle/issues)

## 후원

JQSubtitle은 무료고 앞으로도 무료입니다. 도움이 됐고 마음이 내키시면:
[PayPal](https://paypal.me/jqpark) ☕
