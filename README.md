...
- **Three AI engines** — pick one in the app:
  ...
  - **Local AI** — Ollama on your own PC, no key, fully offline. **Experimental:** quality is
    clearly below Gemini and Claude; it will be replaced once a good enough model exists.
...
2. Download `jqsubtitle_v1.1.py` and double-click it, or run:
   ```
   python jqsubtitle_v1.1.py
   ```
...
- **Local AI (free, offline, experimental)** — select it in the app and click Install; it sets up
  Ollama and a ~9 GB model. An NVIDIA GPU with 12 GB+ VRAM is recommended. Translation quality
  is noticeably worse than Gemini and Claude — use it when you need to work offline or want
...
- **AI 엔진 3종** — 프로그램에서 선택:
  ...
  - **로컬 AI** — 내 PC의 Ollama, 키 없음·완전 오프라인. **실험 기능:** 품질이 Gemini·Claude보다
    눈에 띄게 떨어집니다. 쓸 만한 모델이 나오면 교체할 예정입니다.
...
2. `jqsubtitle_v1.1.py` 다운로드 후 더블클릭 또는:
   ```
   python jqsubtitle_v1.1.py
   ```
...
- **로컬 AI (무료·오프라인·실험)** — 프로그램에서 선택하고 설치 버튼을 누르면 Ollama와
  모델(~9GB)이 설치됩니다. NVIDIA GPU VRAM 12GB 이상 권장. 번역 품질이 Gemini·Claude보다
  확실히 떨어지므로, 인터넷 없이 작업해야 하거나 텍스트를 외부로 보내고 싶지 않을 때
  쓰세요.
