...
if ($LASTEXITCODE -ne 0) {
    Write-Host "    CUDA runtime not installed - JQSubtitle will still work on CPU (slower)." -ForegroundColor Yellow
    Write-Host "    CUDA 런타임 설치 실패 - CPU로도 동작합니다 (느림)." -ForegroundColor Yellow
}
